...
describe('session-start-soul hook', () => {
  ...
  describe('when server returns SOUL.md', () => {
    let server;
    let port;
        ...
        server = createServer((req, res) => {
          if (req.url === '/memory/soul' && req.method === 'GET') {
            ...
            res.end(JSON.stringify({
              status: 'ok',
              data: { content: MOCK_SOUL, filePath: 'SOUL.md' },
            }));
          } else {
    ...
    it('handles large SOUL.md content correctly', async () => {
      ...
      const output = JSON.parse(stdout);
      expect(output.hookSpecificOutput.additionalContext.length).toBeGreaterThan(MOCK_SOUL.length);
      // PREFACE adds at least 100 chars of framing
      expect(output.hookSpecificOutput.additionalContext.length).toBeGreaterThanOrEqual(MOCK_SOUL.length + 100);
    });
  });
});
