...
describe('session-start-memory hook', () => {
  ...
  describe('when server returns MEMORY.md', () => {
    let server;
    let port;
        ...
        server = createServer((req, res) => {
          if (req.url === '/memory/memory' && req.method === 'GET') {
            ...
            res.end(JSON.stringify({
              status: 'ok',
              data: { content: MOCK_MEMORY, filePath: 'MEMORY.md' },
            }));
          } else {
    ...
    it('emits framing preface + <memory>-wrapped content', async () => {
    ...
    it('handles large MEMORY.md content correctly', async () => {
      ...
      const output = JSON.parse(stdout);
      expect(output.hookSpecificOutput.additionalContext.length).toBeGreaterThan(MOCK_MEMORY.length);
      // PREFACE adds at least 100 chars of framing
      expect(output.hookSpecificOutput.additionalContext.length).toBeGreaterThanOrEqual(MOCK_MEMORY.length + 100);
    });
  });
});
