...
try {
  const result = enqueueTranscript({ source: 'stop', input: JSON.parse(raw) });
  if (result.skipped) {
    console.error(`jarvis.session-end.skip: ${result.skipped}`);
  } else {
    console.error(`jarvis.session-end.queued: ${result.queuedAt}`);
  }
} catch (err) {
